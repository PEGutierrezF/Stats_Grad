...
- If _*x*_ is a quantity to be measured that has a normal distribution with mean (\(\mu\)) and standard deviation (\(\sigma\)), we designate this by writing
...
ggplot(data.frame(x, y), aes(x = x, y = y)) +
  ...
  geom_vline(xintercept = mean, color = "red", linetype = "dashed") + geom_vline(xintercept = mean + sd, color = "green", linetype = "dotted") +  geom_vline(xintercept = mean - sd, color = "green", linetype = "dotted") +  labs(title = "", x = "Values", y = "Density")
```
::::
...
- To simplify the comparison of data from different normal distributions, we often convert values to the **standard normal distribution**.
...
- Any normal distribution can be converted into the standard normal distribution by using the **z-score** formula:
...
- This probability is represented by the area under the curve beyond the observed test statistic.

<br>
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width=4, fig.height=2}
# Define mean, standard deviation, and observed test statistic
mean <- 0  # Assume mean is 0 under the null hypothesis
...
ggplot(data, aes(x = x, y = y)) +
  ...
  geom_vline(xintercept = test_stat, color = "red", linetype = "dashed", size = 1.2) +  # Mark the observed test statistic
  geom_ribbon(data = subset(data, x >= test_stat), aes(ymin = 0, ymax = y), fill = "lightblue", alpha = 0.5) +  # Shade area beyond the test statistic
  labs(title = "", x = "Test Statistic", y = "Density") +
  annotate("text", x = test_stat + 0.3, y = 0.05, label = "Observed Test Statistic", color = "red", angle = 90, vjust = 3.5, size=3) +
  theme_minimal() +
...
ggplot(data, aes(x = x, y = y)) +
  ...
  annotate("text", x = test_stat + 0.2, y = 0.05, label = "Test Statistic (z = 1.96)", color = "red", angle = 90, vjust = 3.5, size=2) +
  annotate("text", x = test_stat + 1.2, y = 0.01, label = "p-value < 0.05", color = "black", size = 3) +
  theme_minimal() +
  theme(
...
ggplot(data, aes(x = z, y = density)) +
  ...
  annotate("text", x = 2, y = 0.05, label = "P(z < 0.76)", color = "blue", size = 5) +
...
:::::::::::::: {.columns}
::: {.column width="50%"}
- **Problem Statement**: If the EPA sets the maximum allowable lead concentration in drinking water at 1ppm, and the lead concentrations in public buildings are normally distributed with a mean of 0.6 ppm and a standard deviation of 0.2ppm, **what proportion of public buildings will exceed this threshold and require lead remediation?**
...
prop_above_threshold<-1-pnorm(z_score)
