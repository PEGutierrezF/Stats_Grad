...
dist1 <- dnorm(x_vals, mean = mu, sd = 1)
data1 <- data.frame(x = x_vals, y = dist1)

# Create a data frame for the second plot (normal distribution centered at 0)
dist2 <- dnorm(x_vals, mean = 0, sd = 1)
data2 <- data.frame(x = x_vals, y = dist2)

# First plot: Normal distribution with mean mu
plot1 <- ggplot(data1, aes(x = x, y = y)) +
  geom_line(color = "blue", size = 1) +             # Plot the line
  geom_ribbon(data = subset(data1, x > 0), aes(ymin = 0, ymax = y), fill = "lightblue", alpha = 0.3) + # Fill light blue where x > 0
  labs(x = "Random Variable", y = "Density") +
  scale_x_continuous(breaks = c(mu), labels = c(expression(mu))) + # Label x-axis with mu
  theme_minimal() +
  theme(
...
plot2 <- ggplot(data2, aes(x = x, y = y)) +
  ...
  geom_ribbon(data = subset(data2, x > 0), aes(ymin = 0, ymax = y), fill = "lightblue", alpha = 0.3) + # Fill light blue where x > 0
  labs(x = "Random Variable", y = "Density") +
  scale_x_continuous(breaks = c(0), labels = c("0")) + # Label x-axis with 0
  theme_minimal() +
  theme(
    axis.title.x = element_text(size = 14),
    axis.title.y = element_text(size = 14)
