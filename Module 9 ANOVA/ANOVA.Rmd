...
# Normality
```{r}
byf.shapiro(yield ~ as.character(fertilizer), data=crop)
```

---
# Variance
```{r}
bartlett.test(yield ~ as.character(fertilizer), data=crop)
```
...
ggplot(crop) +
  aes(x = as.character(fertilizer), y = yield, fill = fertilizer) +
  geom_boxplot() +
