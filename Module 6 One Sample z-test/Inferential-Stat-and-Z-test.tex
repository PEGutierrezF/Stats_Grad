...
\begin{itemize}
\item
  In most real-world applications, a \textbf{two-sided} or
  \textbf{two-tailed hypothesis test} is often the most appropriate
  WHY?. This method tests for any difference between the sample and
  population mean. The null and alternative hypotheses are expressed as:

  \[
  H_0: \mu = 170 \quad \text{vs.} \quad H_1: \mu \neq 170
  \]
\item
  To test this hypothesis, we define both rejection and acceptance
  regions. These regions are determined by our chosen significance
  level, typically set at \textbf{0.05}.
\end{itemize}
\end{frame}

\begin{frame}{Types of Hypothesis Tests:}
\phantomsection\label{types-of-hypothesis-tests}
\begin{itemize}
\item
  \textbf{Left-tailed} test: Tests if the sample mean is significantly
  \textbf{less than} the population mean.
\item
  \textbf{Right-tailed} test: Tests if the sample mean is significantly
  \textbf{greater than} the population mean.
\item
  \textbf{Two-tailed} test: Tests if the sample mean is significantly
  different from the population mean, in either direction.
\item
  The \textbf{tails} in a distribution are the \textbf{extreme regions}
  where values of the test statistic indicate significant evidence
  against the null hypothesis, aligning with the alternative hypothesis.
\end{itemize}
\end{frame}
...
\begin{frame}{Expanding on Hypothesis Testing}
\phantomsection\label{expanding-on-hypothesis-testing-3}
\begin{itemize}
\tightlist
\item
  2-tailed tests:
...
\begin{frame}{Basic steps for an \textbf{Inferential Test}}
\phantomsection\label{basic-steps-for-an-inferential-test-3}
\begin{itemize}
\item
  We can select an appropriate test simply by answering some questions.
...
\phantomsection\label{flowcharts}
\begin{figure}
...
\begin{frame}{Flowcharts}
\phantomsection\label{flowcharts-1}
\begin{figure}
...
\begin{frame}{Flowcharts}
\phantomsection\label{flowcharts-2}
...
\begin{frame}{Example: Another example}
\phantomsection\label{example-another-example-4}
