...
- One-tailed test ( $\alpha$= 0.05): Critical value = $\pm$ 1.645.
- Two-tailed test ( $\alpha$= 0.05): $\alpha$ is split into 2.5% in each tail, leading to the widely used = $\pm$ 1.96.
...
| Question Type | Given | Function | Example | Interpretation |
| --- | --- | --- | --- | --- |
| Find probability (area below z-score) (left, smaller, less than) | z-score | `pnorm(z)` | `pnorm(0.76)` → 0.776 | Proportion of area to the left of z = 0.76 |
| Find probability (area above z-score) (right, beyond, larger than) | z-score | `1 - pnorm(z)` | `1 - pnorm(0.76)` → 0.224 | Proportion of area to the right of z = 0.76 |
| Find z-score for left cumulative area (e.g., bottom 5%) | Probability | `qnorm(p)` | `qnorm(0.025)` → -1.96 | Z-score corresponding to the lower 2.5% of the distribution |
| Find z-score for right cumulative area (e.g., top 5%) | Probability | `qnorm(1 - p)` | `qnorm(0.975)` → 1.96 | Z-score corresponding to the upper 2.5% of the distribution |
