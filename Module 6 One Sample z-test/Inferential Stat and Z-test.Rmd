...
A water quality regulation specifies that the lead level in drinking water should be no more than **15 ppb**. The amount of lead in a water sample is random but follows a normal distribution with a mean \( \mu \) and standard deviation \( \sigma \) = 0.25. A random sample of three lead measurements yielded values of 15.84, 15.33, and 15.58. Perform a one-sided z-test to determine if the mean lead level exceeds 15 ppb. 
...
z.test(x, y, alternative='greater', mu=, sigma.x=, sigma.y=NULL,conf.level=.95)
