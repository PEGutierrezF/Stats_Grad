...
```{r echo=FALSE}
set.seed(1234)
wdata = data.frame(data = factor(rep(c("Sample 1", "Sample 2"), each=200)), weight = c(rnorm(200, 55), rnorm(200, 58)))
```
]

.pull-right[.center[
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width=2, fig.height=2.5}
ggplot(wdata, aes(x = weight, fill = data)) +
  geom_histogram(aes(y = ..density..), bins = 30, alpha = 0.6, position = "identity") +
  geom_density(alpha = 0.4) +  # Add density curve
  geom_vline(aes(xintercept = mean(weight)), linetype = "dashed", color = "black") + # Add mean line
  scale_fill_manual(values = c("#00AFBB", "#E7B800")) +  
  theme_minimal() +  theme(
    axis.text.x = element_text(size = 4, color = "gray20"),  
    axis.text.y = element_text(size = 4, color = "gray20"),  
    ...
    legend.position = "top" ) +
  labs(title = "", x = "Weight", y = "Density", fill = "Sample")
```
]]
