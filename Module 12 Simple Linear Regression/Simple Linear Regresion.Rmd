...
p <- ggplot(data = df, aes(x = x, y = y)) +
  ...
  annotate("text", x = 5, y = 8, label = paste0("Y = ", round(intercept, 2), " + ", round(slope, 2), "X"), 
           color = "blue", size = 4)
...
text(x[i] + 0.02, middle_y, labels=expression(e[i] == y[i] - hat(y)[i]), col="blue", pos=4, cex=1.4)
text(x[i] + 0.009, middle_y, "}", srt=0, col="red", cex=3.5)
