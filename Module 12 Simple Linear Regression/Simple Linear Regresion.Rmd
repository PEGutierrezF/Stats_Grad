...
plot( y ~ x , dat, pch = 16, ylab=NA, mgp=c(4,1,.5) , las=1, bty="n", xaxs="i", xpd = TRUE, xlab="Fraction having attained a Bachelor's degree")
abline(M, col="blue")
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.align='top', fig.width=2.5, fig.height=1.75}
...
plot(DBH_in ~ height_ft, data = trees, pch=16, col="gray90", col.axis = "white")
abline(mod, col='blue')

# Add general linear equation as expression
equation_text <- expression(Y == beta[0] + beta[1] * X)
text(x = 72, y = 16.5, labels = equation_text, pos = 4, col = "blue", cex = 0.8)

# Slope triangle
segments(74.8, 12.75, 80, 12.75, col = "purple", lty = 2)  # Vertical line for "changes in y"
segments(80, 12.75, 80, 14.4, col = "purple", lty = 2)  # Horizontal line for "1 unit of increase in x"

text(79, 13, "changes\n in Y", col = "purple", pos = 4, cex = 0.5)
text(77, 13, "1 unit of\n increase in X", col = "purple", pos = 1, cex = 0.5)
```
]
...
# X represents the fraction of residents in each county having attained a
# bachelor's degree or better
par(mar = c(6.5, 10, 0, 0))  # Adjust bottom, left, top, and right margins
dat <- data.frame(
  x = 1:10,
  y = c(3, 5, 9, 13, 12, 15, 21, 22, 22, 26)
)
...
# Create the plot
plot(dat$x, dat$y, pch = 16, col = "blue",
  xlab = "X (Predictor)", ylab = "Y (Response)", 
  main = "",las = 1, bty = "l")
abline(M, col = "blue")

# Highlight and annotate the intercept
plotrix::draw.circle(x=0.7, y=2.7, radius=.2, lwd=1, col = "red")
text(1, 3, "Intercept = 0.87", pos = 4, col = "purple")

# Highlight and annotate the slope
segments(7, 16, 7, 19, col = "purple", lty = 2)  # Horizontal line at slope_y1
segments(7, 16, 6, 16, col = "purple", lty = 2)   # Vertical line to slope_y2
text(7, 17.5, 'Slope:\n 2.53', col = "purple", pos = 4, cex = 1)

# Add the regression equation as symbols
equation_text <- bquote(Y == .(round(intercept, 1)) + .(round(slope, 2)) * X)
text(max(dat$x) - 3, max(dat$y) - 2, equation_text, col = "red", cex = 1.2)

# Add residual lines
fitted_values <- fitted(M)
segments(x0 = dat$x, y0 = dat$y,
x1 = dat$x, y1 = fitted_values,
col = "red", lty = 2
)

# Annotate predicted value example
#predicted_x <- 6
#predicted_y <- intercept + slope * predicted_x
#text(predicted_x, predicted_y + 2, "Predicted Value", col = "blue", pos = 4)
#arrows(predicted_x, predicted_y + 2, predicted_x, predicted_y + 0.5, length = 0.1, col = "blue")
...
Assumptions:
  ...
  - Linear relationship
  - Normally distributed residuals.
      - Are the residuals relatively constant (homoscedastic)?
]
.pull-right[ 
A. Constant distribution (spread) of residuals.

```{r, echo=FALSE, fig.cap="", out.width = '50%'}
knitr::include_graphics("figs/homocedasticity.png")
```
]
...
.pull-right[ 
B. Heteroscedasticity apparent: the spread of residuals increases as the fitted values increase.

```{r, echo=FALSE, fig.cap="", out.width = '50%'}
knitr::include_graphics("figs/heterocedasticity.png")
```
]
...
```{r, echo=FALSE, fig.cap="", out.width = '50%'}
knitr::include_graphics("figs/qqnorm.png")
```
]
...
.pull-right[ 
B. Diagnostic plot of residuals vs fitted values. If the data fit a linear model, the residuals should be grouped near the zero horizontal line. The curved line is a calculated LOESS curve which shows the directions of the distortions from linearity.
Note that the horizontal axis in such a plot consists of the predicted (or 'fitted') values (Y).
.

```{r, echo=FALSE, fig.cap="", out.width = '50%'}
knitr::include_graphics("figs/residual_fittedvalues.png")
```
]
